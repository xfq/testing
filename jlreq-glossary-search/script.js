...
function initialiseGlossarySearch() {
  const searchEnglish = document.getElementById("termlist-search-en");
  const searchJapanese = document.getElementById("termlist-search-ja");
  const table = document.getElementById("termlist-filter");
  const trArray = Array.prototype.slice.call(
    table.querySelectorAll("tbody tr")
  );

  const filterEnglish = event => {
    const searchTerm = event.target.value.toLowerCase();
    trArray.forEach(row => {
      row.classList.add("hidden");
      const tdArray = Array.prototype.slice.call(
  ...
  const filterJapanese = event => {
    ...
    trArray.forEach(row => {
      ...
      tdArray.forEach(cell => {
        if (cell.innerText.indexOf(searchTerm) > -1) {
          row.classList.remove("hidden");
        }
      });
    });
  ...
  searchEnglish.addEventListener("keyup", filterEnglish, false);
  searchJapanese.addEventListener("keyup", filterJapanese, false);
}
